...
if ($result->num_rows > 0) {
    ...
    $investBalance = $row['Balance'];
    $investInterest = $row['interest_rate'];
    $investDate = $row['Date'];

    // finding difference between dates
    $sql_date = "SELECT DATEDIFF('$date_calc', '$investDate') AS date_diff";
    ...
    if ($days_diff >= 30) {
        ...
        while ($days_diff >= 30) {
            $newBalance = $investBalance + $investBalance * $investInterest;
            $days_diff -= 30;
        }
